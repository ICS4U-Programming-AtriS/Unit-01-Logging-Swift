...
for length in LOGLENGTHS { print("\(length) ", terminator: " | ") }
print("]")
...
if let logLengthAsString = readLine() {
    ...
    if let logLength = Double(logLengthAsString) {
        ...
        if LOGLENGTHS.contains(logLength) {
            // Calculate the amount of logs that can fit in the truck
            let logAmount = Double(TRUCKWEIGHTLIMIT) / (Double(WEIGHTPERMETER) * logLength)
